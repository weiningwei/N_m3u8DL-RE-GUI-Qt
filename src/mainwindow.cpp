...
{
    const QString program = m_exePath->text().trimmed();
...
void MainWindow::autoDetectExe()
{
    if (!m_exePath->text().isEmpty())
        return;
    const QString name = "N_m3u8DL-RE.exe";
    QStringList candidates;
    const QString path = QProcessEnvironment::systemEnvironment().value("PATH");
    for (const QString &dir : path.split(';', Qt::SkipEmptyParts))
        candidates.append(QDir(dir).filePath(name));
    candidates.append(QDir::currentPath() + "/" + name);
    candidates.append("D:/Programs/N_m3u8DL-RE/" + name);
    for (const QString &c : candidates) {
        if (QFileInfo::exists(c)) {
            m_exePath->setText(QDir::toNativeSeparators(c));
            return;
        }
    }
}
