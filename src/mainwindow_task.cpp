...
QString MainWindow::presetDirPath()
{
    return QGuiApplication::applicationDirPath() + "/presets";
}
...
void MainWindow::refreshPresetList()
{
    ...
    if (dir.exists()) {
        const auto files = dir.entryList({"*.ini"}, QDir::Files, QDir::Name);
        for (const QString &f : files) {
            QString name = f;
            name.chop(4); // remove .ini
            m_presetCombo->addItem(name);
        }
    }

    if (!m_currentPreset.isEmpty()) {
        int idx = m_presetCombo->findText(m_currentPreset);
...
void MainWindow::savePreset(const QString &name)
{
    ...
    QSettings s(presetDirPath() + "/" + name + ".ini", QSettings::IniFormat);
    // 路径
...
{
    QSettings s(presetDirPath() + "/" + name + ".ini", QSettings::IniFormat);
    if (s.allKeys().isEmpty()) return;

    m_exePath->setText(s.value("exePath", "").toString());
    m_ffmpegPath->setText(s.value("ffmpegPath", "").toString());

    if (m_queueEnabledBox) {
        bool qe = s.value("queueEnabled", false).toBool();
        m_queueEnabledBox->setChecked(qe);
        if (m_maxConcurrentBox) {
            m_maxConcurrentBox->setValue(s.value("maxConcurrent", 5).toInt());
            m_maxConcurrentBox->setEnabled(qe);
        }
    }
    if (m_terminalModeBox)
        m_terminalModeBox->setChecked(s.value("terminalMode", false).toBool());
...
void MainWindow::onPresetSelected(int index)
{
    ...
    // 避免加载时触发保存
    if (name == m_currentPreset) return;
    ...
    if (m_presetNameEdit)
        m_presetNameEdit->setText(m_currentPreset);

    auto s = portableSettings();
    s.setValue("currentPreset", m_currentPreset);
}

void MainWindow::onSavePreset()
{
    if (!m_presetNameEdit) return;
    QString name = m_presetNameEdit->text().trimmed();
    if (name.isEmpty()) {
        QMessageBox::warning(this, "提示", "请输入预设名称。");
        return;
    }
    savePreset(name);
    m_currentPreset = name;
    ...
    appendLog(QString("预设 \"%1\" 已保存。").arg(name));
}

void MainWindow::onDeletePreset()
{
    if (m_currentPreset.isEmpty()) {
        QMessageBox::information(this, "提示", "当前没有活动的预设可删除。");
        return;
    }
    QString file = presetDirPath() + "/" + m_currentPreset + ".ini";
    if (!QFile::exists(file)) return;
    QFile::remove(file);
    m_currentPreset.clear();
    auto s = portableSettings();
    s.setValue("currentPreset", QString());
    if (m_presetNameEdit)
        m_presetNameEdit->clear();
    refreshPresetList();
    m_presetCombo->setCurrentIndex(0);
    appendLog(QString("预设已删除。"));
}
